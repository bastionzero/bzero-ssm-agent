...
// SendStreamDataMessageAndWaitForAck provides a mock function with given fields: ctx, _a1, payloadType, inputData
func (_m *IDataChannel) SendStreamDataMessageAndWaitForAck(ctx context2.Context, _a1 log.T, payloadType contracts.PayloadType, inputData []byte) error {
	ret := _m.Called(ctx, _a1, payloadType, inputData)

	var r0 error
	if rf, ok := ret.Get(0).(func(context2.Context, log.T, contracts.PayloadType, []byte) error); ok {
		r0 = rf(ctx, _a1, payloadType, inputData)
	} else {
		r0 = ret.Error(0)
	}
